...
export default function RecentInstitution({ recentCodes, setRecentCodes: _setRecentCodes, handleSelectWithRecent }: { recentCodes: string[], setRecentCodes: React.Dispatch<React.SetStateAction<string[]>>, handleSelectWithRecent: (school: Institution) => void }) {
    return (
        ...
        >
            <div className="flex justify-between items-center mb-4 border-b border-edge-subtle pb-2">
                ...
                {recentCodes.map((code) => {
                    ...
                    return (
                        <button
                            ...
                            className="p-5 rounded-xl bg-chip border border-edge hover:border-green-500/30 hover:bg-chip-hover transition-all duration-300 group cursor-pointer flex flex-col justify-between h-40 text-left shadow-[var(--shadow-card)] relative overflow-hidden"
                        >
                            <div className="absolute top-0 right-0 w-16 h-16 bg-chip rounded-bl-full pointer-events-none"></div>
                            <div className="flex justify-between items-start w-full mb-3">
                                <UniversityCrest code={school.code} />
                                <span className="bg-chip text-secondary border border-edge font-mono text-[9px] font-bold px-2 py-0.5 rounded uppercase">
                                    {school.code}
                                </span>
                            </div>
                            <div>
                                <h3 className="text-sm font-bold text-heading group-hover:text-heading transition-colors tracking-tight line-clamp-2 leading-snug">
                                    {school.name}
                                </h3>
                                <div className="text-[11px] font-mono text-green-400 group-hover:text-green-300 transition-colors mt-2.5 flex items-center gap-1">
                                    ...
                                    <ExternalLink className="w-3 h-3 text-faint group-hover:text-green-300/60 transition-colors" />
                                </div>
                            </div>
                        </button>
                    );
