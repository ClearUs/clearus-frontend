...
export function ActiveSearching() {
    return (
        <div className="flex items-center justify-center py-8 gap-2 text-xs font-mono text-muted">
...
export function FoundInstitutes({ handleSelectWithRecent, setSearchQuery, inst }: { handleSelectWithRecent: (school: Institution) => void, setSearchQuery: (value: React.SetStateAction<string>) => void, inst: Institution }) {
    return (
        <button
            ...
            className="w-full text-left px-5 py-4 min-h-12 hover:bg-chip transition-colors flex items-center justify-between group cursor-pointer"
        >
            <div className="flex items-center gap-3">
                ...
                <div>
                    <div className="text-sm font-bold text-heading group-hover:text-green-400 transition-colors">
                        {inst.name}
                    </div>
            ...
            <ArrowRight className="w-4 h-4 text-faint group-hover:text-green-400 group-hover:translate-x-1 transition-all" />
        </button>
    )
}
