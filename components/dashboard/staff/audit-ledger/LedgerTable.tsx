  ...
  return (
    <div className="overflow-x-auto w-full no-scrollbar">
      <table className="w-full text-left border-collapse min-w-225">
        <thead>
          <tr className="bg-surface-inset border-b border-edge-subtle text-[10px] text-faint uppercase tracking-wider font-mono">
        ...
        <tbody className="divide-y divide-edge-subtle text-xs font-sans">
          {filteredLogs.length === 0 ? (
            <tr>
              <td colSpan={5} className="px-6 py-12 text-center text-xs text-faint font-mono italic">
            ...
            filteredLogs.map((log) => {
              ...
              return (
                <tr key={log.id} className="hover:bg-chip transition-colors group">
                  {/* TIMESTAMP */}
                  <td className="px-6 py-4.5 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                      <Calendar className="w-3.5 h-3.5 text-faint group-hover:text-emerald-400 transition-colors" />
                      <span className="font-mono text-[11px] text-secondary font-semibold">{formatUTC}</span>
                    </div>
                  </td>

                  {/* ACTOR CARD */}
                  <td className="px-6 py-4.5 whitespace-nowrap">
                    <div className="text-xs font-bold text-secondary">{log.actorName}</div>
                    <div className="text-[10px] text-muted font-mono mt-0.5">{log.actorEmail}</div>
                  ...
                  <td className="px-6 py-4.5">
                    <div className="text-xs font-semibold text-secondary max-w-60 truncate" title={log.targetResource}>
                      {log.targetResource}
                    </div>
                    <div className="text-[10px] text-faint font-mono flex items-center gap-1.5 mt-0.5">
                      <span>Ref ID:</span>
                      <span className="font-bold select-all text-muted">{log.targetId}</span>
                    </div>
                  </td>

                  {/* SECURITY IP NET trail */}
                  <td className="px-6 py-4.5">
                    <div className="flex items-center gap-1.5 font-mono text-[10px] text-secondary mb-0.5">
                      <Globe className="w-3 h-3 text-subtle" />
                      <span className="select-all font-bold">{log.ipAddress}</span>
                    </div>
                    <div className="text-[9px] text-faint truncate max-w-65 flex items-center gap-1" title={log.userAgent}>
